...
public:
    virtual const BidOffer& GetBestBidOffer(const string &productId,OrderBook <T> &data) {
        Order offer_order=data.GetOfferStack()[0];
        Order bid_order=data.GetOfferStack()[0];
        BidOffer *bid_offer = new BidOffer(bid_order,offer_order);
        return *bid_offer;
    }
    virtual void OnMessage(OrderBook<T> & data){}
    
    void ProcessOrderBook(OrderBook<T> & data){
        string ele = data.GetProduct().GetTicker();
